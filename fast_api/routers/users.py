...
router = APIRouter(
    prefix='/users',  # Hash usada para modelagem
    tags=['users'],  # organização do mesmo dominio
)
...
def update_user(
    ...
    current_user.email == user.email  # type: ignore
    current_user.username == user.username  # type: ignore
    current_user.password == get_password_hash(user.password)  # type: ignore
